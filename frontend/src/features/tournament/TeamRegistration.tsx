...
export const TeamRegistration: React.FC<TeamRegistrationProps> = ({
  ...
  const handleRandomize = () => {
    ...
    if (teams.length === 0) {
      ...
      // We can't assign members immediately because we need team IDs.
      // But since socket events are async, we might have a race condition if we try to assign immediately.
      // A better approach for MVP:
      // 1. Create teams if needed (this part is tricky with async).
      // Let's assume teams are created or we wait for them.
      // Actually, let's just use the existing teams if they exist, or create them and wait?
      // The simplest way is to require teams to be created first OR handle it in one go on server (not supported yet).
      // Let's just create teams if count < numTeams.

      // Wait, if we emit create, we won't have IDs back instantly in this function scope.
      // So we should probably split this: "Auto-Create Teams" button (existing) and "Randomize Members" button.
      // But the requirement says "when adding is complete and press divide, it will randomize".
      // So let's assume teams are already created or we create them and then the user has to click again?
      // No, that's bad UX.

      // Workaround: If no teams, we can't assign members yet.
      // Let's make "Randomize" also trigger "Auto Create Teams" if needed, but we'll have to wait for state update.
      // Since we can't wait easily here without refactoring, let's just use the existing handleAutoCreateTeams logic
      // but we can't chain it easily.

      // Alternative: Just distribute to "Team 1", "Team 2" etc conceptually? No, need IDs.

      // Let's stick to: If teams exist, distribute. If not, warn user or auto-create and ask to click again?
      // Or better: Just rely on the fact that we can update the memberInputs state locally and then save?
      // But we need team IDs for memberInputs keys.

      // Let's try to use the existing teams. If 0 teams, we auto-create them.
      // But we can't assign members to non-existent teams.

      // Let's change the flow:
      // 1. User enters names.
      // 2. User clicks "Randomize".
      // 3. If no teams, we create them.
      // 4. We set a flag "pendingRandomization" to true.
      // 5. In useEffect, if "pendingRandomization" is true and teams exist, we distribute and save.
    }

    setPendingRandomization(true);
    setPendingNames(shuffled);
  ...
  const [pendingRandomization, setPendingRandomization] = useState(false);
  const [pendingNames, setPendingNames] = useState<string[]>([]);

  useEffect(() => {
    if (pendingRandomization && teams.length > 0 && pendingNames.length > 0) {
      const numTeams = parseInt(
        localStorage.getItem("tournament.numTeams") || "4"
      );
      ...
      if (teams.length >= numTeams) {
        // Distribute names
        const newMemberInputs = { ...memberInputs };

        // Initialize inputs for all teams
        teams.forEach((team) => {
          if (!newMemberInputs[team.id]) {
            newMemberInputs[team.id] = [];
          }
        });

        // Clear existing? Or append? Let's overwrite for "Randomize"
        teams.forEach((team) => {
          newMemberInputs[team.id] = [];
        });
        ...
        pendingNames.forEach((name, index) => {
          ...
          newMemberInputs[team.id].push(name);
        });

        setMemberInputs(newMemberInputs);

        // Save to server
        teams.forEach((team) => {
          // Clear existing on server
          team.members?.forEach((member: any) => {
            if (socket)
              socket.emit("team:remove_member", {
                teamId: team.id,
                memberId: member.id,
              });
          });
          ...
          // Add new
          const teamMembers = newMemberInputs[team.id];
          teamMembers.forEach((name) => {
            if (socket)
              socket.emit("team:add_member", {
                teamId: team.id,
                memberName: name,
              });
          });
        ...
        setPendingRandomization(false);
        setPendingNames([]);
      }
    }
  }, [pendingRandomization, teams, pendingNames, socket, memberInputs]);
